...
def SnappyDecompress_VLD_RLD_DeltaDecode():
	efa = EFA([])
	...
	Operands:
	1. Input LM offset
	2. Input LM size (in bytes)
	3. Output LM offset
	'''
	tran = state.writeTransition("basic_with_action", state, state0, "SnappyDecompress_procedure")
	#Set SBP (LM Mode)
	tran.writeAction(f"add X8 X7 X5")  	    # X5(SBP in sp) = X16 + X7
	tran.writeAction(f"add X9 X8 X30")  	# X30 (MaxSBP)= X9(InputSize) + X16(SBP) 
	#Set output ptr
	...
	tran.writeAction("addi X1 X29 0") # save cont word
	...
	tran.writeAction(f"sendr_wcont X29 X29 {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")
	tran.writeAction("yieldt")

	# The input block is compressed
	#UDPR_0/2/4/{OUTPUT_PTR_REG} are used by snappy
	tran = state0.writeTransition("commonCarry",state0, state1, 1)

	tran = state1.writeTransition("flagCarry_with_action", state1, state2, 0)
	tran.writeAction("movlr 0(X5) UDPR_0 0 1")
	tran.writeAction("lastact")


	for i in range(0, 237, 4):
		#====== tag : length - 1 00
		tran = state2.writeTransition("commonCarry_with_action", state2, state1, i)
		#        tran.writeAction("print 'CopyLiteral first'")
		length = (i>>2) + 1
		#tran.writeAction("copy_imm SBPB {OUTPUT_PTR_REG} "+str(length))  #deprecated
		tran.writeAction(f"bcpylli X5 {OUTPUT_PTR_REG} "+str(length))
	...
	label = 60 << 2
	tran = state2.writeTransition("commonCarry_with_action", state2, state1, label)
	#    tran.writeAction("print 'CopyLiteral 2'")
	...
	tran.writeAction(f"copy X5 {OUTPUT_PTR_REG} UDPR_0")     #or bcpyll
	#====== get one step back, UDP increases SBPB 1 step automatically
	#tran.writeAction("subi SBPB SBPB 1")   #deprecated
	#tran.writeAction("movir UDPR_1 1")
	#tran.writeAction("sub X5 UDPR_1 X5")
	...
	label = 61 << 2
	tran = state2.writeTransition("commonCarry_with_action", state2, state1, label)
	#    tran.writeAction("print 'CopyLiteral 3'")
	#tran.writeAction("get_bytes SBPB  UDPR_0 2")   #deprecated
	tran.writeAction("movlr 0(X5) UDPR_0 1 2")
	tran.writeAction("addi UDPR_0  UDPR_0 1")
	tran.writeAction(f"copy X5 {OUTPUT_PTR_REG} UDPR_0")  #or bcpyll
	...
	for i in range(1, 256, 4):
		tran = state2.writeTransition("commonCarry_with_action", state2, state1, i)
		#        tran.writeAction("print 'CopyMatchTag01'")
		length = (i >> 2 & 0x7) + 4
		...
		tran.writeAction(f"sub {OUTPUT_PTR_REG} UDPR_0 UDPR_4")
		#tran.writeAction("copy_from_out_imm UDPR_4 {OUTPUT_PTR_REG} "+str(length))  #deprecated
		tran.writeAction(f"copy_imm UDPR_4 {OUTPUT_PTR_REG} "+str(length))
		#tran.writeAction("subi SBPB SBPB 1")     #deprecated
		#tran.writeAction("movir UDPR_1 1")
		#tran.writeAction("sub X5 UDPR_1 X5")
		tran.writeAction("lastact")
	...
	for i in range(2, 256, 4):
		tran = state2.writeTransition("commonCarry_with_action", state2, state1, i)
		#        tran.writeAction("print 'CopyMatchTag10'")
		length = (i - 2 >> 2) + 1
	...
	efa.appendBlockAction("end_of_input_terminate_efa_snappydecompress",f"sub {OUTPUT_PTR_REG} {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")       	#{OUTPUT_PTR_REG}: will hold the output size after sub , {INIT_OUTPUT_PTR_REG} : hold the output pointer 
	efa.appendBlockAction("end_of_input_terminate_efa_snappydecompress",f"sendr_wcont X29 X29 {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")       #Xtemp => X16 
	efa.appendBlockAction("end_of_input_terminate_efa_snappydecompress","yieldt")
	efa.linkBlocktoState("end_of_input_terminate_efa_snappydecompress",state0)
	...
	stateVLD0 = State()
	stateVLD0.alphabet = [0]
	efa.add_state(stateVLD0)

	stateVLD1 = State()
	stateVLD1.alphabet = [0]
	efa.add_state(stateVLD1)

	stateVLD2 = State()
	stateVLD2.alphabet = [0,1]
	efa.add_state(stateVLD2)

	stateVLD3 = State()
	stateVLD3.alphabet = [0]
	efa.add_state(stateVLD3)

	tran = state.writeTransition("epsilonCarry_with_action", state, stateVLD0, "VLD_procedure")
	...
	tran = stateVLD0.writeTransition("commonCarry",stateVLD0, stateVLD1, 0)

	tran = stateVLD1.writeTransition("flagCarry_with_action",stateVLD1, stateVLD2, 0)
	...
	tran = stateVLD2.writeTransition("commonCarry",stateVLD2, stateVLD1, 0)

	tran = stateVLD2.writeTransition("epsilonCarry_with_action",stateVLD2, stateVLD3, 1)
	...
	tran = stateVLD3.writeTransition("epsilonCarry_with_action",stateVLD3, stateVLD0, 0)
	tran.writeAction(f"movir UDPR_7 0")
	tran.writeAction(f"movir UDPR_2 0")
	tran.writeAction("lastact")   
	...
	efa.linkBlocktoState("end_of_input_terminate_efa_VLD",stateVLD0)
	...
	efa.linkBlocktoState("end_of_input_terminate_efa",stateRLD0)
	...
	tran.writeAction(f"movlr 0(X5) UDPR_2 0 {ISSUE_WIDTH}")
	tran.writeAction(f"add UDPR_2 UDPR_1 UDPR_1")
	tran.writeAction(f"movrl UDPR_1 0({OUTPUT_PTR_REG}) 1 {STORE_WIDTH}")
	#tran.writeAction("print '[TFORM]: output: %lu ' X17")
	tran.writeAction("lastact")
	...
	efa.appendBlockAction("end_of_input_terminate_efa_deltadecode",f"sub {OUTPUT_PTR_REG} {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")  #{OUTPUT_PTR_REG}: will hold the output size after sub , {INIT_OUTP>
