...
DeltaEncode_procedure="DeltaEncode_procedure"
RLE_procedure="RLE_procedure"
VLE_procedure="VLE_procedure"
OUTPUT_PTR_REG = "X31"
...
def DeltaEncode_VLE_RLE():
	...
	tran = state.writeTransition("commonCarry", state, state0, DeltaEncode_procedure)
	...
	efa.appendBlockAction("end_of_input_terminate_efa_DeltaEncode",f"sub {OUTPUT_PTR_REG} {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")  #{OUTPUT_PTR_REG}: will hold the output size after sub , {INIT_OUTPUT_PTR_REG} >
	efa.appendBlockAction("end_of_input_terminate_efa_DeltaEncode","yieldt")
	efa.linkBlocktoState("end_of_input_terminate_efa_DeltaEncode",state0)




	stateRLE0 = State()
	stateRLE0.alphabet = [0]
	efa.add_state(stateRLE0)

	stateRLE1 = State()
	stateRLE1.alphabet = [0]
	efa.add_state(stateRLE1)

	stateRLE2 = State()
	stateRLE2.alphabet = [0]
	efa.add_state(stateRLE2)

	stateRLE3 = State()
	stateRLE3.alphabet = [0,1]
	efa.add_state(stateRLE3)    

	stateRLE4 = State()
	stateRLE4.alphabet = [0]
	efa.add_state(stateRLE4)

	stateRLE5 = State()
	stateRLE5.alphabet = [0]
	efa.add_state(stateRLE5)

	tran = state.writeTransition("commonCarry_with_action", state, stateRLE0, RLE_procedure)
	...
	tran = stateRLE0.writeTransition("epsilonCarry_with_action",stateRLE0, stateRLE1, 0)
	tran.writeAction(f"movlr 0(X5) UDPR_2 0 {RLE_ISSUE_WIDTH}")
	tran.writeAction("lastact")   
		
	tran = stateRLE1.writeTransition("commonCarry",stateRLE1, stateRLE2, 0)
	tran.writeAction("ori X23 X23 0")
	tran.writeAction("lastact")


	tran = stateRLE2.writeTransition("flagCarry_with_action",stateRLE2, stateRLE3, 0)
	tran.writeAction(f"movlr 0(X5) UDPR_3 0 {RLE_ISSUE_WIDTH}")
	tran.writeAction(f"sub UDPR_3 UDPR_2 UDPR_4")
	tran.writeAction(f"comp_eq UDPR_4 UDPR_0 0")
	tran.writeAction("lastact")   
		
	tran = stateRLE3.writeTransition("epsilonCarry_with_action",stateRLE3, stateRLE4, 1)
	tran.writeAction(f"addi UDPR_1 UDPR_1 1")
	tran.writeAction("lastact")   
		
	tran = stateRLE3.writeTransition("epsilonCarry_with_action",stateRLE3, stateRLE5, 0)
	tran.writeAction(f"movrl UDPR_1 0({OUTPUT_PTR_REG}) 1 {RLE_STORE_WIDTH}")
	tran.writeAction(f"movrl UDPR_2 0({OUTPUT_PTR_REG}) 1 {RLE_STORE_WIDTH}")
	tran.writeAction(f"print '[TFORM]: output: %lu %lu' X17 X18")
	tran.writeAction(f"addi UDPR_3 UDPR_2 0")
	tran.writeAction(f"movir UDPR_1 1")
	tran.writeAction("lastact")   
		
	tran = stateRLE4.writeTransition("commonCarry",stateRLE4, stateRLE2, 0)
	tran.writeAction("ori X23 X23 0")
	tran.writeAction("lastact")
		
	tran = stateRLE5.writeTransition("commonCarry",stateRLE5, stateRLE2, 0)
	tran.writeAction("ori X24 X24 0")
	tran.writeAction("lastact")

	#    efa = streaming_event_tx(efa,self.RLE_ISSUE_WIDTH, self.adv_width, self.tform_store_width, self.debug)
	#    efa = shared_blocks(efa, state0, RLE_ISSUE_WIDTH, RLE_ADV_WIDTH, RLE_STORE_WIDTH, false)

	efa.appendBlockAction("end_of_input_terminate_efa_RLE",f"sub {OUTPUT_PTR_REG} {INIT_OUTPUT_PTR_REG} {OUTPUT_PTR_REG}")       	#X31: will hold the output size after sub , X30 : hold the
	efa.appendBlockAction("end_of_input_terminate_efa_RLE","yieldt ")
	efa.linkBlocktoState("end_of_input_terminate_efa_RLE",stateRLE0)
