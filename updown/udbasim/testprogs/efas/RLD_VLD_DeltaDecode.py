...
DeltaDecode_procedure="DeltaDecode_procedure"
RLD_procedure="RLD_procedure"
VLD_procedure="VLD_procedure"
OUTPUT_PTR_REG = "X31"
...
def RLD_VLD_DeltaDecode():
	...
	#tran.writeAction(f)
	#Set CR_Issue
	tran.writeAction(f"movir X28 {RLD_ISSUE_WIDTH}")
	tran.writeAction("sli X28 X27 36")  	                            # X27: CR_Issue
	tran.writeAction(f"or X29 X27 X29")  	                            # Update CR_Issue
	#Set CR_Advance
	tran.writeAction(f"movir X28 {RLD_ADV_WIDTH}")
	tran.writeAction(f"sli X28 X27 32")  	                            # X27: CR_Advance
	tran.writeAction(f"or X29 X27 X29")  	                            # Update CR_Advance
	#Set MaxSBP
	tran.writeAction(f"or X29 X30 X4")  	                            # Write MaxSBP(in X30) to SBCR(X4)
	...
	tran.writeAction(f"movlr 0(X5) UDPR_1 0 {RLD_ISSUE_WIDTH}")
	tran.writeAction("lastact")   

	tran = stateRLD1.writeTransition("epsilonCarry_with_action",stateRLD1, stateRLD2, 0)
	tran.writeAction(f"movlr 0(X5) UDPR_2 0 {RLD_ISSUE_WIDTH}")
	tran.writeAction("lastact")   

	tran = stateRLD2.writeTransition("flagCarry_with_action",stateRLD2, stateRLD3, 0)
	tran.writeAction(f"movrl UDPR_2 0({OUTPUT_PTR_REG}) 1 {RLD_STORE_WIDTH}")
