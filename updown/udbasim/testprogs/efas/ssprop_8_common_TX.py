...
def ssprop_8_common_TX():
    ...
    tran.writeAction("ssprop default")
    ...
    tran.writeAction("yieldt")
    tran = state0.writeTransition("defaultCarry", state0, state2, "default")

    tran = state2.writeTransition("basic_with_action", state2, state0, 98)
    tran.writeAction("movir X17 998")
    tran.writeAction("yieldt")
    tran = state2.writeTransition("commonCarry_with_action", state2, state1, 99)
    tran.writeAction("movir X17 999")
    tran.writeAction("yieldt")
    tran = state2.writeTransition("commonCarry_with_action", state2, state1, 100)
    tran.writeAction("movir X17 1100")
    tran.writeAction("yieldt")
    tran = state2.writeTransition("commonCarry_with_action", state2, state1, 101 )
    tran.writeAction("movir X17 1101")
    tran.writeAction("yieldt")
    tran = state2.writeTransition("basic", state2, state3, "default")

    for i in range (200):
        tran = state3.writeTransition("commonCarry_with_action", state3, state1, i)
        tran.writeAction(f"movir X17 {i}")
        tran.writeAction("yieldt")
