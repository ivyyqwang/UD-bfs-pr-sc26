...
class TranslationEntryInstaller:
    def __init__(self, state, enable_debug=False):
        ...
        self.ud_free_ev_label = "DRAMalloc::updown_translation_free"
    ...
    def __gen_bcst(self):
        ...
        ud_free_tran    = self.state.writeTransition("eventCarry", self.state, self.state, self.ud_free_ev_label)
        ...
        Operands:   X8: virtual base address
                    ...
                    X14: continuation
                    X15: allocation or free flag, 0: allocate, 1: free

        '''
        alloc_fail_label = "alloc_fail"
        free_alloc_label = "free_alloc"
        skil_free_alloc_label = "skip_free_alloc"
        glb_bcst_tran.writeAction(f"beqi {'X15'} {1} {free_alloc_label}")
        if self.debug_flag:
        ...
        glb_bcst_tran.writeAction(f"{free_alloc_label}: addi {'X12'} {num_child} {0}")
        if self.debug_flag:
            glb_bcst_tran.writeAction(f"print ' '")
            glb_bcst_tran.writeAction(f"print '[DEBUG][NWID %d] Free allocation request to segment base_addr=0x%lx, broadcast free to all nodes' {'X0'}  {'X8'}")
        set_ev_label(glb_bcst_tran, ev_word, self.node_bcst_ev_label, new_thread = True)
        set_ev_label(glb_bcst_tran, cont, self.glb_bcst_ret_ev_label, src_ev="X2")
        broadcast(glb_bcst_tran, ev_word, num_child, cont, \
            (LOG2_LANE_PER_UD + LOG2_UD_PER_NODE), f"{'X8'} 8", self.scratch, 'ops', flag='free')
        glb_bcst_tran.writeAction(f"addi {'X14'} {cont} 0")
        glb_bcst_tran.writeAction("yield")
        ...
        node_bcst_tran.writeAction(f"beqi {'X15'} {1} {free_alloc_label}")
        set_ev_label(node_bcst_tran, ev_word, self.ud_install_ev_label, new_thread = True)
        node_bcst_tran.writeAction(f"jmp {skil_free_alloc_label}")
        set_ev_label(node_bcst_tran, ev_word, self.ud_free_ev_label, new_thread = True, label=free_alloc_label)
        set_ev_label(node_bcst_tran, cont, self.node_bcst_ret_ev_label, src_ev="X2", label=skil_free_alloc_label)
        broadcast(node_bcst_tran, ev_word, num_child, cont, (LOG2_LANE_PER_UD), f"X8 8", self.scratch, 'ops')
        ...
        '''
        Event:      Remove the translation entry to the updown
        Operands:   X8: virtual base address
                    X9: size of the segment
                    X10: swizzle mask
                    X11: physical base address
        ...
        continue_label  = "continue"
        if self.debug_flag and self.print_level > 1:
            ud_free_tran.writeAction(f"print ' '")
            ud_free_tran.writeAction(f"print '[DEBUG][NWID %d] Event <dramalloc_updown_translation_free>  " +
                                        f"Free translation entry va_base = %lu(0x%lx) pa_base = %lu(0x%lx) size = %lu' " +
                                        f"{'X0'} {'X8'} {'X8'} {'X11'} {'X11'} {'X9'}")
        ud_free_tran.writeAction(f"beqi {'X10'} {0} {local_trans_label}")
        ud_free_tran.writeAction(f"instrans X8 X11 X9 X10 3 {0b11}")
        ud_free_tran.writeAction(f"jmp {continue_label}")
        ud_free_tran.writeAction(f"{local_trans_label}: instrans X8 X11 X9 X10 2 {0b11}")
        ud_free_tran.writeAction(f"{continue_label}: sendr_reply X8 X14 {self.scratch[0]}")
        ud_free_tran.writeAction("yield_terminate")
...
class DRAMalloc:
    ...
    def __gen_dramalloc(self):
        ...
        if self.debug_flag:
            ...
            dramalloc_tran.writeAction(f"print '[DEBUG][NWID %d] Send DRAM free request for segment starting at %lu(0x%lx) to DRAMalloc at addr %lu(0x%lx)' {'X0'} {'X10'} {'X10'} {req_ptr} {req_ptr}")
        dramalloc_tran.writeAction(f"yield")
...
def dramalloc_linkable(efa):
    ...
    DRAMalloc(state, enable_debug=False)
