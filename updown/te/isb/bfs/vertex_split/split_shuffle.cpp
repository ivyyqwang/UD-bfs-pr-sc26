...
int main(int argc, char *argv[]) {
    ...
    uint64_t min_orig_vid = UINT64_MAX;
    ...
    while (std::getline(inputFile, line)) {
        ...
        min_orig_vid = std::min(min_orig_vid, std::min(src, dst));
    ...
    printf("Max original vertex id: %ld\n", max_orig_vid);
    printf("Min original vertex id: %ld\n", min_orig_vid);
    ...
    if (no_split) {
        for (const std::pair<uint64_t, uint64_t> &v : orig_vertices) {
            split_bases[v.first] = new_vid;
            num_split_vids[v.first] = 1;
            reverse_map[new_vid] = v.first;
            updated_degree_map[new_vid] = 0;
            new_vid++;
#ifdef DEBUG
            printf("Vertex %ld degree %ld remaps to vid %ld. \n", v.first, v.second, new_vid);
#endif
        }
    } else {
        for (const std::pair<uint64_t, uint64_t> &v : orig_vertices) {
            ...
            if (num_split == 1) {
                split_bases[v.first] = new_vid;
                num_split_vids[v.first] = num_split;
                reverse_map[new_vid] = v.first;
                updated_degree_map[new_vid] = 0;
                new_vid ++;
            } else {
                ...
                for (uint64_t i = 0; i < num_split; i++) {
                    updated_degree_map[new_vid + i] = 0;
                    reverse_map[new_vid + i] = v.first;
                }
                new_vid += num_split;
            }

#ifdef DEBUG
            printf("Vertex %ld degree %ld split into %ld vertices, starting %ld. \n", v.first, v.second, num_split, new_vid);
#endif
            if (stats) max_orig_degree = std::max(max_orig_degree, v.second);
        }
    
    }
    uint64_t num_orig_vertices = max_orig_vid + 1;
    orig_vertices.clear();

#ifdef DEBUG
    for (uint64_t i = 0; i < new_vid; i++) {
    ...
    uint64_t num_src_split, num_dst_split;
    ...
    if (no_split) {
        for (auto &e : edges) {
            new_edges[i] = Edge(split_bases[e.src], split_bases[e.dst]);
            i++;
            updated_degree_map[split_bases[e.src]] += 1;
        }
    } else {
        ...
        for (auto &e : edges) {
            src = e.src;
            dst = e.dst;
            if (num_split_vids[src] == 1) {
                src_split = split_bases[src];
            } else {
                src_split = split_bases[src] + (rand() % num_split_vids[src]);
            }
            if (num_split_vids[dst] == 1) {
                dst_split = split_bases[dst];
            } else {
                dst_split = split_bases[dst] + (rand() % num_split_vids[dst]);
            }
    ...
    for (uint64_t i = 0; i < num_split_vertices; i++) {
        ...
        orig_vid = reverse_map[i];
        vertices[i].split_range = (split_bases[orig_vid] << 32) | (split_bases[orig_vid] + num_split_vids[orig_vid]);
        vertices[i].offset = 0;
#ifdef DEBUG
        printf("Vertex %ld - degree %ld - orig_vid %ld - split_range %ld\n", vertices[i].vid, vertices[i].degree, vertices[i].orig_vid, vertices[i].split_range);
#endif
