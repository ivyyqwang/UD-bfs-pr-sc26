...
#include "reset_deg_exe.hpp"

#define VALIDATE_RESULT

#define USAGE "USAGE: ./reset_deg <gv_bin> <nl_bin> <num_lanes> <k>"
...
uint64_t reset_deg_cpu(uint64_t num_verts, vertex_t *g_v_bin, int64_t k)
{
...
int main(int argc, char* argv[]) {
	...
	UpDown::BASimUDRuntime_t* rt = new UpDown::BASimUDRuntime_t(machine, "reset_deg_exe.bin", 0, 100);
    
    /* Initialize DRAM memory allocator */
    dramalloc::DramAllocator *allocator = new dramalloc::DramAllocator(rt, reset_deg_exe::DRAMalloc__global_broadcast /*eventlabel*/);
    ...
    ops.set_operand(4, log2_num_control_lanes_per_level);
    ops.set_operand(5, 0);
    ops.set_operand(6, k);

    UpDown::event_t event(reset_deg_exe::reset_deg_master__start, nwid, UpDown::CREATE_THREAD, &ops); // generate start event
    ...
    #ifdef VALIDATE_RESULT
    uint64_t num_empty_cpu = reset_deg_cpu(num_verts, g_v_bin, k);
    uint64_t j = 0;
    uint64_t* sa1;
    for (uint64_t i = 0; i < num_verts; i++) {
    ...
    if(num_empty_updown == num_empty_cpu){
        printf("Reset_deg Finished (No Errors!)\n"); fflush(stdout);
    }else{
        printf("num_empty_updown (%lu) != num_empty_cpu (%lu)", num_empty_updown, num_empty_cpu); fflush(stdout);
    }
    #endif
    printf("Final K: %lu\n", k);
