...
#define USAGE "USAGE: ./pagerankMSRsplit <graph_file_gv.bin> <graph_file_nl.bin> <num_nodes> <num_uds> <partition_per_lane> (<output_file_path>)\n\
...
/*---------------------------- Input Parameter ----------------------------*/
  ...
  int PART_PARM = atoi(argv[5]);

  if (num_nodes < 2) {
    num_nodes = 1;
    num_uds_per_node = atoi(argv[4]);
  } 
