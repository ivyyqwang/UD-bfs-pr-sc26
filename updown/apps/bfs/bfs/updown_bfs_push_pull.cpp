...
void init_scratchpad(UpDown::UDRuntime_t *rt, Vertex *g_v, Vertex *orig_g_v_bin, uint64_t *front_base, uint64_t num_vertices, uint64_t num_uds,
  ...
  /**
    ...
    OB_4:   Original graph vertex array pointer.
  ...
  **/
  ...
  ops.set_operand(4, (uint64_t)orig_g_v_bin);
...
void bfs_init_itr(UpDown::UDRuntime_t *rt, Vertex *g_v, Vertex *orig_g_v_bin, uint64_t **front_bins, uint64_t num_uds, uint64_t num_vertices, uint64_t root_id,
  ...
  Vertex *orig_root = (Vertex *)allocator->translate_udva2sa((uint64_t)(orig_g_v_bin + root->orig_vid));
  orig_root->distance = 0;
  printf("Update the distance of original vertex %ld for root vertex to %ld.\n", root->orig_vid, root->distance);
...
int main(int argc, char *argv[]) {
  ...
  if (((int)num_uds >> 5) >= 1)
    bfs_rt = new UpDown::BASimUDRuntime_t(machine, "push_pull_exe.bin", 0, 100, std::string(graph_file) + "_" + std::to_string(((int)num_uds >> 5)) + "nds");
  else
    bfs_rt = new UpDown::BASimUDRuntime_t(machine, "push_pull_exe.bin", 0, 100, std::string(graph_file) + "_" + argv[2] + "uds");
#else
...
#ifdef GEM5_MODE
  dramalloc::DramAllocator *allocator = new dramalloc::DramAllocator(bfs_rt push_pull_exe::DRAMalloc__global_broadcast /*eventlabel*/, blockSize);
#else
  dramalloc::DramAllocator *allocator = new dramalloc::DramAllocator(bfs_rt, push_pull_exe::DRAMalloc__global_broadcast /*eventlabel*/);
#endif
  ...
  std::size_t n;
  uint64_t num_orig_vertices, num_split_vertices, num_edges = 0;
  fseek(in_file, 0, SEEK_SET);
  n = fread(&num_orig_vertices, sizeof(num_orig_vertices), 1, in_file);
  n = fread(&num_split_vertices, sizeof(num_split_vertices), 1, in_file);
  n = fread(&num_edges, sizeof(num_edges), 1, in_file);

  printf("Graph of Size :\n\tnum_orig_vertices=%ld\n\tnum_split_vertices=%ld\n\tnum_edges=%ld\n", num_orig_vertices, num_split_vertices, num_edges);
  ...
  Vertex *orig_g_v_bin = reinterpret_cast<Vertex *>(allocator->mm_malloc_global(num_orig_vertices * sizeof(Vertex), blockSize, machine.NumNodes, 0));
  ...
  Vertex *temp_vertex, *temp_orig_vertex;
  uint64_t *neighbors, *temp_neigh;
  uint64_t max_degree = 0;
  // Create original vertices
  for (int i = 0; i < num_orig_vertices; i++) {
    temp_orig_vertex = (Vertex *)allocator->translate_udva2sa((uint64_t)(orig_g_v_bin + i));
    temp_orig_vertex->vid = i;
    temp_orig_vertex->degree = 0;
    temp_orig_vertex->distance = 0xffffffffffffffff;
    temp_orig_vertex->parent = 0xffffffffffffffff;
  }
  // Read in spilit vertices
  for (int i = 0; i < num_split_vertices; i++) {
    ...
    temp_orig_vertex = (Vertex *)allocator->translate_udva2sa((uint64_t)(orig_g_v_bin + temp_vertex->orig_vid));
    temp_orig_vertex->split_range = temp_vertex->split_range;
    temp_orig_vertex->degree += temp_vertex->degree;
    // printf("Update the degree of original vertex %ld to %ld.\n",
    //        temp_vertex->orig_vid, temp_orig_vertex->degree);
    fflush(stdout);
  }
  // Read in edges
  for (int i = 0; i < num_split_vertices; i++) {
    ...
    for (int j = 0; j < temp_vertex->degree; j++) {
      temp_neigh = (uint64_t *)allocator->translate_udva2sa((uint64_t)(temp_vertex->neighbors + j));
      // printf("%ld, ", *temp_neigh);
      printf("(%ld, %ld) \t", temp_vertex->orig_vid, *temp_neigh);
    }
    printf("]\n");
#endif
...
#ifdef DEBUG
    printf("Partition %d: begin vid %ld\n", i, iterator_offset);
#endif
    temp_iterator = (Iterator *)allocator->translate_udva2sa((uint64_t)(partitions + i));
    temp_iterator->begin = g_v_bin + std::min(iterator_offset, num_split_vertices);
    iterator_offset += num_vertices_per_part;
    temp_iterator->end = g_v_bin + std::min(iterator_offset, num_split_vertices);
#ifdef DEBUG
    printf("Partition %d: end vid %ld\n", i, iterator_offset);
#endif
#ifdef DEBUG
  ...
  for (int i = 0; i < num_uds; i++) {
    ...
    *sa_frontier = frontier + ((num_split_vertices / num_uds) * k * 2);
    sa_frontier = (uint64_t **)allocator->translate_udva2sa((uint64_t)(front_bins + k + 1));
    *sa_frontier = frontier + ((num_split_vertices / num_uds) * (k + 1) * 2);
#ifdef DEBUG
    printf("Allocating frontier for updown %d starting from addr %p and addr %p\n", i, frontier + ((num_split_vertices / num_uds) * k * 2), *sa_frontier);
#endif
  }
  ...
  bfs_init_itr(bfs_rt, g_v_bin, orig_g_v_bin, front_bins, num_uds, num_split_vertices, root_vertex_id, allocator);
  fflush(stdout);
  ...
  printf("Initialize scratchpad...\n");
  // Init scratchpad
  init_scratchpad(bfs_rt, g_v_bin, orig_g_v_bin, sa_frontier[0], num_split_vertices, num_uds, front_stride, partitions, num_partition_per_lane);
  ...
  for (int i = 0; i < num_split_vertices; i++) {
    temp_vertex = (Vertex *)allocator->translate_udva2sa((uint64_t)(g_v_bin + i));
    if (temp_vertex->distance == 0 && temp_vertex->degree == 0)
      continue;
  ...
  printf("==========================\nOriginal vertices\n=============================\n");
  for (int i = 0; i < num_orig_vertices; i++) {
    temp_vertex = (Vertex *)allocator->translate_udva2sa((uint64_t)(orig_g_v_bin + i));
    if (temp_vertex->distance < 0 && temp_vertex->degree == 0)
      continue;
    printf("Original vertex %d (addr %p) - deg %ld, dist %ld, parent %ld\n", i, (orig_g_v_bin + i), temp_vertex->degree, temp_vertex->distance,
           temp_vertex->parent);
    fflush(stdout);
  }
#endif
  ...
  if (validate_result) {
    ...
    for (int i = 0; i < num_orig_vertices; i++) {
      ...
      if (temp_vertex->distance < 0 && temp_vertex->degree == 0)
        continue;
      if (temp_vertex->distance == 0xffffffffffffffff)
        continue;
      // #ifdef DEBUG
      // printf("Vertex %ld - dist %ld\n", temp_vertex->vid, temp_vertex->distance);
      // #endif
      output << "Vertex " << i << " - dist " << temp_vertex->distance << std::endl;
    }
  }
#endif
