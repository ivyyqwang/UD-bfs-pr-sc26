...
def run_sssp_once(graph_file, graph_format, source_vertex, warmup_iter=2, test_iter=5):
    if graph_format == "txt":
    ...
    print("SSSP source vertex:", source_vertex)

    for _ in range(warmup_iter):
        sssp_result = cugraph.sssp(G, source=source_vertex)
    ...
    reached_vertices = int((sssp_result["distance"] != -1).sum())
    max_distance = int(sssp_result["distance"].max())
...
def main():
    if len(sys.argv) < 3:
        print("Usage:")
        print("  python sssp_batch.py <graph_dir> <graph_k_list_file>")
        print("  python sssp_batch.py <graph_dir> <graph1> <k1> [<graph2> <k2> ...]")
        print("Example:")
        print("  python sssp_batch.py ./datasets graph_k_list.txt")
        print("  # where each row is: graph_name, k_value")
        print("  # k_value is used as SSSP source vertex")
        print("  python sssp_batch.py ./datasets graph_a.tsv 4 graph_b 6")
        sys.exit(1)
    ...
    for idx, (graph_name, k_value) in enumerate(graph_k_pairs, start=1):
        ...
        try:
            ...
            run_sssp_once(graph_path, graph_format, k_value)
