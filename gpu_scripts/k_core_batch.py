...
def run_k_core_once(tsv_file, warmup_iter=2, test_iter=5):
    G = load_tsv_to_cugraph(tsv_file)

    print("Number of vertices:", G.number_of_vertices())
    print("Number of edges:", G.number_of_edges())

    for _ in range(warmup_iter):
        kcore_result = cugraph.k_core(G)

    t_total = 0.0
    for i in range(test_iter):
        t0 = time.time()
        if i == 0:
            nvtx.RangePush("k_core")
        _ = cugraph.k_core(G)
        t1 = time.time()
        if i == 0:
            nvtx.RangePop()
    ...
    result_vertices = kcore_result.number_of_vertices()
    result_edges = kcore_result.number_of_edges()

    print(f"k-core complete.")
    print("k-core vertices:", result_vertices)
    print("k-core edges:", result_edges)
...
def main():
    if len(sys.argv) < 3:
        print("Usage:")
        print("  python k_core_batch.py <graph_dir> <graph_k_list_file>")
        print("  python k_core_batch.py <graph_dir> <graph1> <k1> [<graph2> <k2> ...]")
        print("Example:")
        print("  python k_core_batch.py ./datasets graph_k_list.txt")
        print("  # where each row is: graph_name, k_value")
        print("  python k_core_batch.py ./datasets graph_a.tsv 4 graph_b 6")
        sys.exit(1)
