...
def run_k_truss_once(graph_file, graph_format, k, warmup_iter=2, test_iter=5):
    if graph_format == "txt":
    ...
    for _ in range(warmup_iter):
        ktruss_result = cugraph.k_truss(G, k)

    t_total = 0.0
    for i in range(test_iter):
        t0 = time.time()
        if i == 0:
            nvtx.RangePush("k_truss")
        _ = cugraph.k_truss(G, k)
        t1 = time.time()
        if i == 0:
            nvtx.RangePop()
        t_total += t1 - t0

    result_edges = ktruss_result.number_of_edges()
    result_vertices = ktruss_result.number_of_vertices()

    print(f"k-truss complete (k={k}).")
    print("k-truss vertices:", result_vertices)
    print("k-truss edges:", result_edges)
...
def main():
    if len(sys.argv) < 3:
        print("Usage:")
        print("  python k_truss_batch.py <graph_dir> <graph_k_list_file>")
        print("  python k_truss_batch.py <graph_dir> <graph1> <k1> [<graph2> <k2> ...]")
        print("Example:")
        print("  python k_truss_batch.py ./datasets graph_k_list.txt")
        print("  # where each row is: graph_name, k_value")
        print("  python k_truss_batch.py ./datasets graph_a.tsv 4 graph_b 6")
        sys.exit(1)
    ...
    for idx, (graph_name, k_value) in enumerate(graph_k_pairs, start=1):
        print("\n" + "=" * 72)
        print(f"[{idx}/{len(graph_k_pairs)}] Graph='{graph_name}', k={k_value}")

        try:
            graph_path, graph_format = resolve_graph_path(graph_dir, graph_name)
            print(f"Using file: {graph_path}")
            run_k_truss_once(graph_path, graph_format, k_value)
        except Exception as exc:
            print(f"Failed for graph '{graph_name}' with k={k_value}: {exc}")
