...
def run_connected_components_once(tsv_file, warmup_iter=2, test_iter=5):
    ...
    for i in range(test_iter):
        t0 = time.time()
        if i == 0:
            nvtx.RangePush("connected_components")
        cc_result = cugraph.connected_components(G)
        t1 = time.time()
        if i == 0:
            nvtx.RangePop()
    ...
    num_components = int(cc_result["labels"].nunique())
...
def main():
    if len(sys.argv) < 3:
        print("Usage:")
        print("  python connected_components_batch.py <graph_dir> <graph_k_list_file>")
        print("  python connected_components_batch.py <graph_dir> <graph1> <k1> [<graph2> <k2> ...]")
        print("Example:")
        print("  python connected_components_batch.py ./datasets graph_k_list.txt")
        print("  # where each row is: graph_name, k_value")
        print("  # k_value is validated but ignored for connected components")
        print("  python connected_components_batch.py ./datasets graph_a.tsv 4 graph_b 6")
        sys.exit(1)
    ...
    for idx, (graph_name, k_value) in enumerate(graph_k_pairs, start=1):
        ...
        print(
            ...
            f"(ignoring k={k_value} for connected components)"
        )

        try:
            graph_path = resolve_graph_path(graph_dir, graph_name)
            print(f"Using file: {graph_path}")
            run_connected_components_once(graph_path)
