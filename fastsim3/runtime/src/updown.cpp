...
UDRuntime_t::UDRuntime_t(ud_machine_t machineConfig) : MachineConfig(machineConfig) {
  ...
  UDRuntime_t::UDRuntime_t(ud_machine_t machineConfig, uint64_t nodeID, uint64_t topPerNode) : MachineConfig(machineConfig) {
  UPDOWN_INFOMSG("Initializing runtime with custom machineConfig");
  initialize(nodeID, topPerNode);
}

void UDRuntime_t::initialize(uint64_t nodeID_, uint64_t topPerNode_) {
  calc_addrmap();
  MappedMemoryManager = new ud_mapped_memory_t(this->MachineConfig);
    ...
    this->nodeID = nodeID_;
    this->topPerNode = topPerNode_;
    set_barrier_times(this->nnodes);
  #endif
}
...
void UDRuntime_t::calc_addrmap() {
  ...
  UPDOWN_INFOMSG("calc_addrmap: maddr: 0x%lX spaddr: 0x%lX ctrlAddr: 0x%lX progAddr: 0x%lX", 
    reinterpret_cast<uint64_t>(BaseAddrs.mmaddr), reinterpret_cast<uint64_t>(BaseAddrs.spaddr), 
    reinterpret_cast<uint64_t>(BaseAddrs.ctrlAddr), reinterpret_cast<uint64_t>(BaseAddrs.progAddr));
}
