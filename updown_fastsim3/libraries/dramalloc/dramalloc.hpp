...
public:
  ...
  /**
   * \brief Translates a virtual address to a simulated address.
   *
   * \param virt The virtual address.
   * \return The simulated address. Null if the translation fails.
   */
  void *translate_udva2sa(uint64_t virt);

  /**
   * \brief Translates a virtual address to the UD physical address.
   *
   * \param virt The virtual address.
   * \return The UD physical address. Null if the translation fails.
   */
  void *translate_udva2pa(uint64_t virt);
  
  /**
   * \brief Get the traslation entry parameters with input virtual address virt. If not found, return false; else return true and update virt_base, phy_base, blockSize, numNodes.
   *
   * \param virt The virtual address.
   * \return If not found, return false; else return true and update virt_base, phy_base, blockSize, numNodes
   */
  bool getTranslationEntry(uint64_t virt, uint64_t &virt_base, uint64_t &phy_base, uint64_t &blockSize, uint64_t &numNodes);
