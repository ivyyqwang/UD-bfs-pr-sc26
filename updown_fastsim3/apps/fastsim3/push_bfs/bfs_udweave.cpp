...
typedef struct Vertex {
  ...
  uint64_t distance;  // padding in PR
}vertex_t;
...
/*---------------------------- Input Parameter ----------------------------*/
  ...
  uint64_t globalDRAMRequirement = g_v_bin_size + nlist_beg_size + queue_size;
  #ifdef VALIDATE_RESULT
  localDRAMRequirement = g_v_bin_size + nlist_beg_size + blockSize;
  printf("\tPrivate DRAM: g_v_bin_size: %lu\n\tnlist_beg_size: %lu\n\tblockSize: %lu\n\tSum: %lu\n", g_v_bin_size, nlist_beg_size, blockSize, localDRAMRequirement);
  printf("\tGlobal DRAM: g_v_bin_size: %lu\n\tnlist_beg_size: %lu\n\tfrontier_size: %lu\n\tSum: %lu\n", g_v_bin_size, nlist_beg_size, queue_size, globalDRAMRequirement);
  #else
...
#else
  UpDown::BASimUDRuntime_t* rt = new UpDown::BASimUDRuntime_t(machine, "bfs_udweave_exe.bin", 0);
  ...
  #ifdef GEM5_MODE
    allocator = new dramalloc::DramAllocator(rt, bfs_udweave_exe::DRAMalloc__global_broadcast /*eventlabel*/, blockSize);
  #else
    allocator = new dramalloc::DramAllocator(rt, bfs_udweave_exe::DRAMalloc__global_broadcast /*eventlabel*/);
  #endif
  ...
  /*---------------------------- global pointer ----------------------------*/
  vertex_t* g_v_bin_global_va = NULL;
  uint64_t* nlist_beg_global_va = NULL;
  uint64_t* frontier_global_va = NULL;

  uint64_t g_v_bin_global_virt_base = 0;
  uint64_t nlist_beg_global_virt_base = 0;
  uint64_t frontier_global_virt_base = 0;

  uint64_t g_v_bin_global_phy_base = 0;
  uint64_t nlist_beg_global_phy_base = 0;
  uint64_t frontier_global_phy_base = 0;

  uint64_t tmp_blockSize = 0;
  uint64_t tmp_numNodes = 0;

  uint64_t* global_pointers = reinterpret_cast<uint64_t *>(malloc(9*sizeof(uint64_t)));

  printf("Initialize global pointer\n");  fflush(stdout);

  if(rt->getRank() == 0){
    g_v_bin_global_va = reinterpret_cast<vertex_t *>(allocator->mm_malloc_global(g_v_bin_size, blockSize, num_nodes, 0));
    nlist_beg_global_va = reinterpret_cast<uint64_t*>(allocator->mm_malloc_global(nlist_beg_size, blockSize, num_nodes, 0));
    frontier_global_va = reinterpret_cast<uint64_t *>(allocator->mm_malloc_global(queue_size, blockSize, num_nodes, 0));
    if(g_v_bin_global_va == nullptr || nlist_beg_global_va == nullptr || frontier_global_va == nullptr){
    ...
    if( !allocator->getTranslationEntry(reinterpret_cast<uint64_t>(g_v_bin_global_va), g_v_bin_global_virt_base, g_v_bin_global_phy_base, tmp_blockSize, tmp_numNodes) ||
        ...
        !allocator->getTranslationEntry(reinterpret_cast<uint64_t>(frontier_global_va), frontier_global_virt_base, frontier_global_phy_base, tmp_blockSize, tmp_numNodes) ){
    ...
    global_pointers[2] = reinterpret_cast<uint64_t>(frontier_global_va);
    global_pointers[3] = reinterpret_cast<uint64_t>(g_v_bin_global_virt_base);
    global_pointers[4] = reinterpret_cast<uint64_t>(nlist_beg_global_virt_base);
    global_pointers[5] = reinterpret_cast<uint64_t>(frontier_global_virt_base);
    global_pointers[6] = reinterpret_cast<uint64_t>(g_v_bin_global_phy_base);
    global_pointers[7] = reinterpret_cast<uint64_t>(nlist_beg_global_phy_base);
    global_pointers[8] = reinterpret_cast<uint64_t>(frontier_global_phy_base);


    rt->sendMPIData(reinterpret_cast<void *>(global_pointers), 9*sizeof(uint64_t));

  }else{
    rt->receiveMPIData(reinterpret_cast<void *>(global_pointers), 9*sizeof(uint64_t));
    ...
    frontier_global_va = reinterpret_cast<uint64_t *>(global_pointers[2]);

    g_v_bin_global_virt_base = global_pointers[3];
    nlist_beg_global_virt_base = global_pointers[4];
    frontier_global_virt_base = global_pointers[5];

    g_v_bin_global_phy_base = global_pointers[6];
    nlist_beg_global_phy_base = global_pointers[7];
    frontier_global_phy_base = global_pointers[8];
  }
  ...
  /*---------------------------- Initialize frontier_global ----------------------------*/
  printf("Initialize frontier_global.\n"); fflush(stdout);
  #if not defined GEM5_MODE
  gettimeofday(&start1,NULL);
  #endif

  next_size = rt->getMinNodeID() * blockSize;
  va1 = reinterpret_cast<uint64_t>(frontier_global_va) + next_size;
  va1_end = reinterpret_cast<uint64_t>(frontier_global_va) + queue_size;
  next_size = (num_nodes - nodesPerRuntime) * blockSize;

  while(va1 < va1_end){
    for(uint64_t i=0; i < nodesPerRuntime && va1 < va1_end; i++){
      uint64_t * sa1 = reinterpret_cast<uint64_t *>(rt->mapVA2SA(va1, frontier_global_virt_base, frontier_global_phy_base, blockSize, num_nodes));
      int64_t len = blockSize;
      if((va1 + len) > va1_end){
        len = va1_end - va1;
      }
      memset(sa1, 0, len);
      if(va1 == reinterpret_cast<uint64_t>(frontier_global_va)){
        sa1[0] = 2;
        sa1[1] = 0;
  ...
  gettimeofday(&end1,NULL);
  time = (end1.tv_sec-start1.tv_sec) + (end1.tv_usec-start1.tv_usec) / 1000000.0;
  printf("Initialize frontier_global time:%lf s\n", time); fflush(stdout);
  #endif
  ...
  ops.set_operand(2, (uint64_t)frontier_global_va);
  ...
  printf("start CopyGlobal2Local \n"); fflush(stdout);
  CopyGlobal2Local(rt, allocator, blockSize, (uint64_t)g_v_bin_global_va, (uint64_t)g_v_bin, g_v_bin_size);
  printf("end CopyGlobal2Local \n"); fflush(stdout);

  for(int64_t j = 0; j < num_split_verts; j++){
    int64_t offset = (uint64_t)(g_v_bin[j].neighbors);
    uint64_t *loc_nlist = reinterpret_cast<uint64_t *>(offset + nlist_offset);
    g_v_bin[j].neighbors = loc_nlist;
  }
