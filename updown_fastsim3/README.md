...
This repository contains the UpDown Simulator and other infrastructure.
...
10. `-DUPDOWN_DETAIL_STATS=` enables the collection of detailed statistics including thread invocation histograms, event stats and so on. Activates `NETWORK_STATS`
11. `-DUPDOWN_NETWORK_STATS=` enables the collection of network statistics such as DRAM access communication among the UpDown nodes, etc.
12. `-DUPDOWN_DETAIL_INST_HIST=` enables individual instruction counting and histogram
13. `DEBUG_LOG_LEVEL=<#>` flag is used in addition to `-DUPDOWN_ENABLE_DEBUG` to set how many debug statements are printed, the higher the number the more information is printed
